...
public class CustomerRepo(Context context, UserManager<ApiUser> userManager) : ICustomerRepo
{
    ...
    public async Task<string> CreateAsync(ApiUser customer)
    {
        ...
        try
        {
            var result = await _userManager.CreateAsync(customerEntity, customer.PasswordHash!);
            if(result.Succeeded){
    ...
    public async Task<ApiUser?> RetrieveAsync(string Id)
    {
